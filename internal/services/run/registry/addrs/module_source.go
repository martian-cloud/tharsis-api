...
This file is from https://github.com/hashicorp/terraform/tree/main/internal.
...
Tharsis Modifications:
	...
	- All of the code besides IsModuleSourceLocal() and
	ParseModuleSourceRemote() has been removed. Also, the
	two functions have been made public to meet Tharsis'
	use-case.
	- ParseModuleSourceRemote() renamed to
	ValidateModuleSourceRemote() only returns errors to
	eliminate the dependency on another module in the
	addrs package. Also, Tharsis only expects errors
	and never uses the return value.
	- Since the functions mentioned above are public,
...
import (
	...
	"gitlab.com/infor-cloud/martian-cloud/tharsis/tharsis-api/internal/services/run/registry/getmodules"
)
...
// IsModuleSourceLocal returns true is a local module is used.
...
// ValidateModuleSourceRemote determines if moduleSource is remote.
func ValidateModuleSourceRemote(raw string) error {
	var subDir string
	raw, subDir = getmodules.SplitPackageSubdir(raw)
	if strings.HasPrefix(subDir, "../") {
		return fmt.Errorf("subdirectory path %q leads outside of the module package", subDir)
	}
	...
	if moreSubDir != "" {
		...
		if strings.HasPrefix(subDir, "../") {
			...
			// downstream.
			return fmt.Errorf("detected subdirectory path %q of %q leads outside of the module package", subDir, norm)
		}
	}
