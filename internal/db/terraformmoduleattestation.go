...
//go:generate go tool mockery --name TerraformModuleAttestations --inpackage --case underscore
...
func (t *terraformModuleAttestations) CreateModuleAttestation(ctx context.Context, moduleAttestation *models.TerraformModuleAttestation) (*models.TerraformModuleAttestation, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to marshal module attestation digests", errors.WithSpan(span))
	}
		...
		if pgErr := asPgError(err); pgErr != nil {
			if isUniqueViolation(pgErr) {
				...
				case "index_terraform_module_attestations_on_module_and_data_sha_sum":
					return nil, errors.New("another module attestation with the same data already exists for this module", errors.WithErrorCode(errors.EConflict), errors.WithSpan(span))
				default:
