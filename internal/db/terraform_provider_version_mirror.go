...
//go:generate go tool mockery --name TerraformProviderVersionMirrors --inpackage --case underscore
...
func (t *terraformProviderVersionMirrors) CreateVersionMirror(ctx context.Context, versionMirror *models.TerraformProviderVersionMirror) (*models.TerraformProviderVersionMirror, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to marshal provider version mirror digests", errors.WithSpan(span))
	}
	...
	if err != nil {
		if pgErr := asPgError(err); pgErr != nil {
			if isUniqueViolation(pgErr) {
				return nil, errors.New("terraform provider version is already mirrored", errors.WithErrorCode(errors.EConflict), errors.WithSpan(span))
			}
		}
		return nil, errors.Wrap(err, "failed to execute query", errors.WithSpan(span))
	}
