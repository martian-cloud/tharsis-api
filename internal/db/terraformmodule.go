...
const (
	...
	TerraformModuleSortableFieldCreatedAtAsc        TerraformModuleSortableField = "CREATED_AT_ASC"
	TerraformModuleSortableFieldCreatedAtDesc       TerraformModuleSortableField = "CREATED_AT_DESC"
...
func (ts TerraformModuleSortableField) getFieldDescriptor() *pagination.FieldDescriptor {
	...
	case TerraformModuleSortableFieldCreatedAtAsc, TerraformModuleSortableFieldCreatedAtDesc:
		return &pagination.FieldDescriptor{Key: "created_at", Table: "terraform_modules", Col: "created_at"}
...
func (t *terraformModules) CreateModule(ctx context.Context, module *models.TerraformModule) (*models.TerraformModule, error) {
	...
	if labelsErr != nil {
		return nil, errors.Wrap(labelsErr, "failed to marshal labels to JSON", errors.WithSpan(span))
	}
	...
	if err != nil {
		if pgErr := asPgError(err); pgErr != nil {
			if isUniqueViolation(pgErr) {
				return nil, errors.New("terraform module with name %s and system %s already exists", module.Name, module.System, errors.WithErrorCode(errors.EConflict), errors.WithSpan(span))
			}
		}
		return nil, errors.Wrap(err, "failed to execute query", errors.WithSpan(span))
	}
