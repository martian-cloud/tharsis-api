...
func getPlanCacheObjectKey(run *models.Run) string {
	return fmt.Sprintf("workspaces/%s/runs/%s/plan/%s", run.WorkspaceID, run.Metadata.ID, run.Plan.GetID())
}

func getPlanJSONObjectKey(run *models.Run) string {
	return fmt.Sprintf("workspaces/%s/runs/%s/plan/%s.json", run.WorkspaceID, run.Metadata.ID, run.Plan.GetID())
}

func getPlanDiffObjectKey(run *models.Run) string {
	return fmt.Sprintf("workspaces/%s/runs/%s/plan/diff_%s.json", run.WorkspaceID, run.Metadata.ID, run.Plan.GetID())
}
