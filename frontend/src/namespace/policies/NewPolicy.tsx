...
import PolicyForm, { buildApproverInput, buildKindDataInput, DEFAULT_POLICY_FORM_DATA, hasRequiredKindData, isMissingApprovers, PolicyFormData } from './PolicyForm';
...
function NewPolicy({ ownerId, ownerPath, groupPath }: Props) {
    ...
    const onCreate = () => {
        ...
        if (!formData.kind || !formData.name || !hasRequiredKindData(formData) || isMissingApprovers(formData)) return;
        setError(undefined);
        ...
        <Box>
            ...
            <Box marginTop={2}>
                <Button
                    ...
                    disabled={!formData.kind || !formData.name || !hasRequiredKindData(formData) || isMissingApprovers(formData)}
                    variant="outlined"
                    color="primary"
                    sx={{ marginRight: 2 }}
                    onClick={onCreate}
